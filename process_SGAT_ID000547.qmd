---
title: SGAT processing for ID000547
author: Somebody
date: today
format: 
  html: 
...
:::{.callout-note}
Removing weird late dawns in June
:::
...
raw_filtered <- filter_date(raw, first = "2020-06-20", last = NULL)
...
check_adjust(raw_filtered, adjust = c(110, 120, 130, 140))
```

> Choose the one that looks best and apply that.

```{r adjusting_times_apply, fig.cap = "Adjusted data"}
raw_adjust <- apply_adjust(raw_filtered, 140)
...
:::{.callout-note}
Threshold is a bit high to accommodate the end of the data shifts.
:::

```{r twighlight_threshold}
threshold <- nighttime_light(raw_adjust, q = 0.59)
threshold
```
...
:::{.callout-note}
Late summer seems to be relatively stable and before migration, but also pick later start to avoid bump, maybe due to breeding?
:::

```{r model_calibration, fig.asp = 1.5}
calib <- calibration(raw_adjust, t_smooth, dates = c("2020-07-20", "2020-09-01"))
```
...
```{r initial_paths}
coords <- initial_path(t_smooth, calib, tol = 0.18, 
